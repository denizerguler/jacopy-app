...
st.logo("docs/tutorials/jacopy_logo.png", size="large")
...
tabs = st.tabs([
    ...
    L("VII · BDCP (Ateşli–Esen–Sütlü)", "VII · BDCP (Ateşli–Esen–Sütlü)"),
    L("VIII · 3-Lie Algebroids (Ateşli–Esen–Sütlü)", "VIII · 3-Lie Algebroidleri (Ateşli–Esen–Sütlü)"),
])
...
with tabs[4]:
    ...
    with proof_tabs5[0]:
        ...
        with st.expander(L("📖 Mathematical Proof", "📖 Matematiksel Kanıt"), expanded=False):
            ...
            st.markdown(L(
                "**On 0-forms $f$:** since $\\iota_V f = 0$:",
                "**0-formlarda:** $\\iota_V f = 0$ olduğundan:"
            ))
            ...
            st.markdown(L(
                "**Duality:** $K$ and $\\tilde{K}$ are exact duals of one another:",
                "**Dualite:** $K$ ve $\\tilde{K}$ birbirinin tam dualidir:"
            ))
            ...
            st.markdown(L(
                "Both operators are the exact building blocks used in the derivator identities §3.1.5 — see the next two tabs.",
                "Her iki operatör de §3.1.5 derivator kimliklerinin yapı taşlarıdır — sonraki iki sekmeye bakın."
            ))
            st.download_button(
                L("⬇ Download as LaTeX (.tex)", "⬇ LaTeX olarak indir (.tex)"),
                '\\documentclass{amsart}\n\\usepackage{amsmath,amssymb,geometry}\n\\geometry{margin=2.5cm}\n\\begin{document}\n\\section*{K and K-tilde Operators}\n\\subsection*{K Operator (Form Side)}\n\\begin{equation*}\nK_V := -\\mathcal{L}_V + d\\circ\\iota_V = -(\\iota_V\\circ d)\n\\end{equation*}\nOn 0-forms $f$: since $\\iota_V f = 0$,\n\\begin{equation*}\nK_V(f) = -V(f).\n\\end{equation*}\n\\subsection*{K-tilde Operator (Multivector Side)}\n\\begin{equation*}\n\\tilde{K}_\\eta := -\\tilde{\\mathcal{L}}_\\eta + \\tilde{d}\\circ\\tilde{\\iota}_\\eta = -(\\tilde{\\iota}_\\eta\\circ\\tilde{d})\n\\end{equation*}\n\\textbf{Duality:} $K\\leftrightarrow\\tilde{K}$, $d\\leftrightarrow\\tilde{d}$, $\\iota\\leftrightarrow\\tilde{\\iota}$, $\\mathcal{L}\\leftrightarrow\\tilde{\\mathcal{L}}$.\n\\end{document}',
    ...
    with proof_tabs5[1]:
        ...
        with st.expander(L("📖 Mathematical Proof", "📖 Matematiksel Kanıt"), expanded=False):
            ...
            st.markdown(L(
                "**Step 1:** Recall $\\tilde{d}V=[\\pi,V]_{SN}$ and $\\tilde{\\iota}_\\eta V=[\\eta,V]_{SN}$:",
            ...
            st.markdown(L(
                "**Step 2:** Apply the graded Jacobi identity for the SN bracket (with $|\\pi|=|\\eta|=1$):",
                "**Adım 2:** SN bracket için derecelendirilmiş Jacobi kimliğini uygula ($|\\pi|=|\\eta|=1$):"
            ))
            st.latex(r"[\pi,[\eta,V]_{SN}]_{SN}+[\eta,[\pi,V]_{SN}]_{SN} = [[\pi,\eta]_{SN},V]_{SN}")
            st.markdown(L(
                "**Step 3:** By definition of the tilde Lie derivative, $[[\\pi,\\eta]_{SN},V]_{SN} = \\tilde{\\mathcal{L}}_\\eta V$:",
                "**Adım 3:** Tilde Lie türevi tanımından $[[\\pi,\\eta]_{SN},V]_{SN} = \\tilde{\\mathcal{L}}_\\eta V$:"
            ))
            st.latex(r"= \tilde{\mathcal{L}}_\eta V \qquad \square")
            st.download_button(
                L("⬇ Download as LaTeX (.tex)", "⬇ LaTeX olarak indir (.tex)"),
                '\\documentclass{amsart}\n\\usepackage{amsmath,amssymb,geometry}\n\\geometry{margin=2.5cm}\n\\begin{document}\n\\section*{Tilde Magic Formula}\n\\begin{proof}\n\\textbf{Step 1.} Expand using $\\tilde{d}V=[\\pi,V]_{SN}$, $\\tilde{\\iota}_\\eta V=[\\eta,V]_{SN}$:\n\\begin{equation*}\n(\\tilde{d}\\tilde{\\iota}_\\eta+\\tilde{\\iota}_\\eta\\tilde{d})V = [\\pi,[\\eta,V]]_{SN}+[\\eta,[\\pi,V]]_{SN}\n\\end{equation*}\n\\textbf{Step 2.} Graded Jacobi identity for SN ($|\\pi|=|\\eta|=1$):\n\\begin{equation*}\n= [[\\pi,\\eta]_{SN},V]_{SN}\n\\end{equation*}\n\\textbf{Step 3.} By definition of the tilde Lie derivative:\n\\begin{equation*}\n= \\tilde{\\mathcal{L}}_\\eta V \\qquad \\square\n\\end{equation*}\n\\end{proof}\n\\end{document}',
                file_name="tilde_magic_formula_proof.tex",
                mime="text/plain",
                key="dl_v2",
            )
        st.markdown("---")
    ...
    with proof_tabs5[2]:
        ...
        with st.expander(L("📖 Mathematical Proof", "📖 Matematiksel Kanıt"), expanded=False):
            ...
            st.markdown(L(
                "This measures the failure of the linear map $\\Phi$ to be a derivation of the bracket $[\\cdot,\\cdot]$.",
                "Bu, doğrusal dönüşüm $\\Phi$'nin bracket $[\\cdot,\\cdot]$'ın bir türevi olamamasını ölçer."
            ))
            st.markdown(L("### Identity (1) — Key Steps", "### Kimlik (1) — Temel Adımlar"))
            st.markdown(L(
                "**Step 1:** Expand the LHS using the Koszul bracket formula "
            ...
            st.markdown(L(
                "**Step 2:** Commute $\\mathcal{L}_U$ past the anchor $\\pi^\\sharp$ using the key identity:",
                "**Adım 2:** Anahtar kimlikle $\\mathcal{L}_U$'yu anchor $\\pi^\\sharp$'ın yanından geçir:"
            ))
            st.latex(r"\mathcal{L}_U(\pi^\sharp\eta) - \pi^\sharp(\mathcal{L}_U\eta) = \tilde{K}_\eta U")
            st.markdown(L(
                "**Step 3:** After 109 full expansions of the Koszul bracket, Cartan magic formula, and the K-tilde identity, all terms collect exactly into the RHS:",
                "**Adım 3:** Koszul bracket, Cartan magic formülü ve K-tilde kimliğinin 109 tam açılımından sonra tüm terimler tam olarak sağ tarafa toplanır:"
            ))
            st.latex(r"= \mathcal{L}_{\tilde{K}_\eta U}\mu + K_{\tilde{K}_\mu U}\eta \qquad \square")
            st.caption(L(
                "Jacopy's engine handles all 109 steps automatically — click the button below to run the full proof.",
                "Jacopy'nin motoru tüm 109 adımı otomatik olarak işler — tam kanıtı çalıştırmak için aşağıdaki butona tıklayın."
            ))
            st.download_button(
                L("⬇ Download as LaTeX (.tex)", "⬇ LaTeX olarak indir (.tex)"),
                '\\documentclass{amsart}\n\\usepackage{amsmath,amssymb,geometry}\n\\geometry{margin=2.5cm}\n\\begin{document}\n\\section*{Derivator Identity (1) --- Form Side}\n\\subsection*{Derivator Definition}\n\\begin{equation*}\n\\mathcal{D}_\\Phi(\\eta,\\mu) := \\Phi[\\eta,\\mu] - [\\Phi\\eta,\\mu] - [\\eta,\\Phi\\mu]\n\\end{equation*}\n\\subsection*{Identity (1)}\n\\begin{equation*}\n\\mathcal{D}^{T^*M}_{\\mathcal{L}_U}(\\eta,\\mu) = \\mathcal{L}_{\\tilde{K}_\\eta U}\\mu + K_{\\tilde{K}_\\mu U}\\eta\n\\end{equation*}\n\\begin{proof}\nExpand the LHS using the Koszul bracket formula, commute $\\mathcal{L}_U$ past the anchor $\\pi^\\sharp$ using the key identity $\\mathcal{L}_U(\\pi^\\sharp\\eta)-\\pi^\\sharp(\\mathcal{L}_U\\eta)=\\tilde{K}_\\eta U$, and collect all 109 terms to obtain the RHS. $\\square$\n\\end{proof}\n\\end{document}',
    ...
    with proof_tabs5[3]:
        ...
        with st.expander(L("📖 Mathematical Proof", "📖 Matematiksel Kanıt"), expanded=False):
            ...
            st.markdown(L(
                "This identity is obtained from Identity (1) by replacing every object with its tilde counterpart:",
                "Bu kimlik, Kimlik (1)'den her nesneyi tilde karşılığıyla değiştirerek elde edilir:"
            ))
            table_rows = [
                ("$d$", "$\\tilde{d}$"),
                ("$\\iota$", "$\\tilde{\\iota}$"),
                ("$\\mathcal{L}$", "$\\tilde{\\mathcal{L}}$"),
                ("$K$", "$\\tilde{K}$"),
                ("Koszul bracket", "SN bracket"),
                ("109 steps", "117 steps"),
            ]
            c1, c2 = st.columns(2)
            c1.markdown(L("**Form side (1)**", "**Form tarafı (1)**"))
            c2.markdown(L("**Multivector side (1')**", "**Çokvektör tarafı (1')**"))
            for form_item, mv_item in table_rows:
                c1, c2 = st.columns(2)
                c1.markdown(form_item)
                c2.markdown(mv_item)
            st.markdown(L(
                "**Why 117 and not 109?** The Schouten–Nijenhuis bracket has more atomic base cases "
                "(vector-vector, vector-function, function-function, plus higher wedge degrees) than the Koszul bracket, "
                "which gives 8 additional expansion paths in the full proof.",
                "**Neden 117 ve 109 değil?** Schouten–Nijenhuis bracket'ının Koszul bracket'tan daha fazla "
                "atomik temel durumu vardır, bu da tam kanıtta 8 ekstra açılım yolu verir."
            ))
            st.download_button(
                L("⬇ Download as LaTeX (.tex)", "⬇ LaTeX olarak indir (.tex)"),
                '\\documentclass{amsart}\n\\usepackage{amsmath,amssymb,geometry}\n\\geometry{margin=2.5cm}\n\\begin{document}\n\\section*{Derivator Identity (1\\'') --- Multivector Side}\n\\begin{equation*}\n\\tilde{\\mathcal{D}}^{SN}_{\\tilde{\\mathcal{L}}_\\eta}(U,V) = \\tilde{\\mathcal{L}}_{K_U\\eta}V + \\tilde{K}_{K_V\\eta}U\n\\end{equation*}\n\\begin{proof}\nExact dual of Identity (1): replace $d\\leftrightarrow\\tilde{d}$, $\\iota\\leftrightarrow\\tilde{\\iota}$, $K\\leftrightarrow\\tilde{K}$, Koszul bracket$\\leftrightarrow$SN bracket. Expand and collect 117 terms (8 more than Identity (1) because the SN bracket has more atomic base cases). $\\square$\n\\end{proof}\n\\end{document}',
    ...
    with proof_tabs5[4]:
        ...
        with st.expander(L("📖 Mathematical Proof", "📖 Matematiksel Kanıt"), expanded=False):
            ...
            st.markdown(L(
                "**Step 1:** Function-level Jacobi, via the Lichnerowicz formula:",
                "**Adım 1:** Lichnerowicz formülüyle fonksiyon seviyesinde Jacobi:"
            ))
            st.latex(r"\mathrm{Jac}_\pi(f,g,h) = \tfrac{1}{2}[\pi,\pi]_{SN}(df,dg,dh)")
            st.markdown(L(
                "**Step 2:** Koszul Jacobi, restricted to exact 1-forms $\\alpha=df$, $\\beta=dg$:",
                "**Adım 2:** Kesin 1-formlarda $\\alpha=df$, $\\beta=dg$ Koszul Jacobi:"
            ))
            st.latex(r"\mathrm{Jac}_{[\cdot,\cdot]_K}(df,dg,dh) = \tfrac{1}{2}[\pi,\pi]_{SN}(df,dg,dh)")
            st.markdown(L(
                "**Step 3:** Both formulas are literally identical — hence they vanish simultaneously:",
                "**Adım 3:** Her iki formül tam olarak aynıdır — dolayısıyla aynı anda sıfırlanırlar:"
            ))
            st.latex(r"\mathrm{Jac}_\pi \equiv 0 \iff \mathrm{Jac}_{[\cdot,\cdot]_K} \equiv 0 \iff [\pi,\pi]_{SN} = 0 \qquad \square")
            st.info(L(
                "**Deep reason:** The Koszul bracket is the **derived bracket** of the SN bracket with respect to $\\pi$. "
                "Both Jacobi identities are therefore manifestations of one single underlying SN-Jacobi statement for $\\pi$.",
                "**Derin neden:** Koszul bracket, $\\pi$'ye göre SN bracket'ın **türetilmiş bracket**'ıdır. "
                "Her iki Jacobi kimliği de, $\\pi$ için tek bir temel SN-Jacobi ifadesinin tezahürüdür."
            ))
            st.download_button(
                L("⬇ Download as LaTeX (.tex)", "⬇ LaTeX olarak indir (.tex)"),
                '\\documentclass{amsart}\n\\usepackage{amsmath,amssymb,geometry}\n\\geometry{margin=2.5cm}\n\\begin{document}\n\\section*{Same Obstruction: Function vs Form Jacobi}\n\\begin{proof}\nBy the Lichnerowicz formula:\n\\begin{equation*}\n\\mathrm{Jac}_\\pi(f,g,h) = \\tfrac{1}{2}[\\pi,\\pi]_{SN}(df,dg,dh) = \\mathrm{Jac}_{[\\cdot,\\cdot]_K}(df,dg,dh)\n\\end{equation*}\nBoth vanish iff $[\\pi,\\pi]_{SN}=0$. Deep reason: the Koszul bracket is the derived bracket of the SN bracket w.r.t.\\ $\\pi$. $\\square$\n\\end{proof}\n\\end{document}',
...
# ══════════════════════════════════════════════════════════════
# TAB VII — Bicocycle Double Cross Product (BDCP) Lie Algebroids
#           Ateşli, Esen & Sütlü, arXiv:2503.12059
#           "On Product Lie Algebroids, and Collective Motion"
#
# Uses the real BDCPSection / BDCPBracket classes defined in
# 32_bdcp_algebroids.py — no ad-hoc bracket tables duplicated here.
# ══════════════════════════════════════════════════════════════
with tabs[6]:
    st.header(L(
        "VII · Bicocycle Double Cross Product Lie Algebroids",
        "VII · Bikosiklik Çift Çapraz Çarpım Lie Algebroidleri"
    ))
    st.caption("Ateşli, Esen & Sütlü — *On Product Lie Algebroids, and Collective Motion*, arXiv:2503.12059 (2025)")

    st.markdown(L(
        """This paper asks the **(de)coupling problem**: when two systems interact mutually
        (not just one acting on the other), what is the *most general* algebraic gadget that
        produces the equations of motion of their combined ("collective") behaviour?

        Its answer is the **bicocycle double cross product (BDCP)** Lie algebroid
        $\\mathcal{A}_\\zeta\\bowtie_\\psi\\mathcal{B}$: given two vector bundles $\\mathcal{A}$, $\\mathcal{B}$
        over the same base, *six* structure maps $(\\phi,\\zeta,\\psi,\\theta,\\rho,\\sigma)$ combine into one
        Lie bracket on $\\mathcal{A}\\times\\mathcal{B}$. Every weaker/older notion — unified product,
        double cross product (matched pair), semi-direct product, 2-cocycle extension — is the
        special case where some of these six maps vanish.""",
        """Bu makale **(ayrıştırma/eşleştirme) problemini** soruyor: iki sistem karşılıklı
        etkileşiyorsa (yalnızca biri diğerini etkilemiyorsa), bu ikisinin birleşik ("kolektif")
        hareketinin hareket denklemlerini üretecek *en genel* cebirsel yapı nedir?

        Cevap, **bikosiklik çift çapraz çarpım (BDCP)** Lie algebroididir:
        $\\mathcal{A}_\\zeta\\bowtie_\\psi\\mathcal{B}$. Aynı taban üzerindeki iki vektör demeti
        $\\mathcal{A}$, $\\mathcal{B}$ için *altı* yapı dönüşümü $(\\phi,\\zeta,\\psi,\\theta,\\rho,\\sigma)$
        $\\mathcal{A}\\times\\mathcal{B}$ üzerinde tek bir Lie bracket'ında birleşir. Daha zayıf/eski
        tüm kavramlar — unified product, double cross product, semi-direct product,
        2-kosiklik genişlemesi — bu altı dönüşümden bazılarının sıfır olduğu özel durumlardır."""
    ))
    st.latex(
        r"[(a_1,b_1),(a_2,b_2)]_{\zeta\bowtie_\psi} = \Big(\phi(a_1,a_2)-\rho(b_2,a_1)+\rho(b_1,a_2)+\psi(b_1,b_2),\ "
        r"\theta(b_1,b_2)-\sigma(a_2,b_1)+\sigma(a_1,b_2)+\zeta(a_1,a_2)\Big)"
    )
    st.markdown("---")

    from jacopy.core.expr import Symbol as BdcpSymbol, Neg as BdcpNeg, Integer as BdcpInteger
    from jacopy.core.properties import Graded as BdcpGraded
    from jacopy.core.registry import PropertyRegistry as BdcpRegistry
    from jacopy.proof import prove_jacobi as bdcp_prove_jacobi
    from jacopy.proof.strategies import ProofFailure as BdcpProofFailure

    # Import the real classes built in 32_bdcp_algebroids.py
    import importlib.util as _ilu
    _bdcp_path = Path.cwd() / "docs" / "tutorials" / "32_bdcp_algebroids.py"
    if not _bdcp_path.is_file():
        _bdcp_path = here / "32_bdcp_algebroids.py"
    _spec = _ilu.spec_from_file_location("bdcp_module", str(_bdcp_path))
    _bdcp_module = _ilu.module_from_spec(_spec)
    import sys as _sys
    _sys.modules["bdcp_module"] = _bdcp_module
    try:
        _spec.loader.exec_module(_bdcp_module)
        BDCPSection = _bdcp_module.BDCPSection
        BDCPBracket = _bdcp_module.BDCPBracket
        _bdcp_import_ok = True
    except Exception as _e:
        _bdcp_import_ok = False
        _bdcp_import_error = _e

    proof_tabs7 = st.tabs([
        L("Hierarchy Explorer", "Hiyerarşi Kaşifi"),
        L("Central extension: Heisenberg", "Merkezi genişleme: Heisenberg"),
        L("Semi-direct product: aff(1)", "Yarı-direkt çarpım: aff(1)"),
        L("What's still needed", "Hâlâ neye ihtiyaç var"),
    ])

    if not _bdcp_import_ok:
        st.error(L(
            f"Could not load 32_bdcp_algebroids.py: {_bdcp_import_error}. "
            "Make sure docs/tutorials/32_bdcp_algebroids.py exists.",
            f"32_bdcp_algebroids.py yüklenemedi: {_bdcp_import_error}. "
            "docs/tutorials/32_bdcp_algebroids.py dosyasının var olduğundan emin olun."
        ))
    else:
        # ── VII.1 Hierarchy Explorer — now backed by BDCPBracket.classify() ──
        with proof_tabs7[0]:
            section_intro(
                L("Which corner of the hierarchy am I in?", "Hiyerarşinin hangi köşesindeyim?"),
                L(
                    "Toggle which of the six structure maps are trivial and the real "
                    "`BDCPBracket.classify()` method (from 32_bdcp_algebroids.py) names the "
                    "resulting product — not a re-derived if/else copy, the same code object "
                    "used by the worked examples below.",
                    "Altı yapı dönüşümünden hangilerinin sıfır olduğunu işaretleyin; gerçek "
                    "`BDCPBracket.classify()` metodu (32_bdcp_algebroids.py'den) ortaya çıkan "
                    "ürünü adlandırsın — aşağıdaki örneklerde kullanılan aynı kod nesnesi."
                )
            )
            c1, c2, c3 = st.columns(3)
            zeta_triv  = c1.checkbox(L("ζ = 0  (no A↦B twisted cocycle)", "ζ = 0  (A↦B kosiklik yok)"), value=True, key="h_zeta")
            psi_triv   = c1.checkbox(L("ψ = 0  (no B↦A twisted cocycle)", "ψ = 0  (B↦A kosiklik yok)"), value=True, key="h_psi")
            sigma_triv = c2.checkbox(L("σ = 0  (A does not act on B)",   "σ = 0  (A, B'ye etki etmiyor)"), value=True, key="h_sigma")
            rho_triv   = c2.checkbox(L("ρ = 0  (B does not act on A)",   "ρ = 0  (B, A'ya etki etmiyor)"), value=False, key="h_rho")
            phi_triv   = c3.checkbox(L("φ = 0  (A itself is abelian)",   "φ = 0  (A'nın kendisi abelyen)"), value=True, key="h_phi")
            theta_triv = c3.checkbox(L("θ = 0  (B itself is abelian)",   "θ = 0  (B'nin kendisi abelyen)"), value=True, key="h_theta")

            def _dummy(x, y, registry=None): return BdcpSymbol("·")
            kwargs = {}
            if not phi_triv:   kwargs["phi"] = _dummy
            if not zeta_triv:  kwargs["zeta"] = _dummy
            if not psi_triv:   kwargs["psi"] = _dummy
            if not theta_triv: kwargs["theta"] = _dummy
            if not rho_triv:   kwargs["rho"] = _dummy
            if not sigma_triv: kwargs["sigma"] = _dummy

            probe = BDCPBracket(**kwargs)
            st.success(f"**{probe.classify()}**")

        # ── VII.2 Heisenberg — built via BDCPBracket, verified via prove_jacobi ──
        with proof_tabs7[1]:
            section_intro(
                L("2-cocycle extension → central extension: the Heisenberg algebra",
                  "2-kosiklik genişleme → merkezi genişleme: Heisenberg cebiri"),
                L(
                    "$\\mathfrak{g}=\\mathrm{span}(Z)$ abelian, $\\mathfrak{h}=\\mathrm{span}(P,Q)$ abelian, "
                    "only nonzero map $\\psi(P,Q)=Z$. Built here as an actual `BDCPBracket(psi=...)` "
                    "instance — its `.classify()` names it, and `.as_custom_bracket_on_flat_symbols()` "
                    "feeds Jacopy's own verified `prove_jacobi` engine (same one used in Tab I).",
                    "$\\mathfrak{g}=\\mathrm{span}(Z)$ abelyen, $\\mathfrak{h}=\\mathrm{span}(P,Q)$ abelyen, "
                    "sıfırdan farklı tek dönüşüm $\\psi(P,Q)=Z$. Burada gerçek bir `BDCPBracket(psi=...)` "
                    "örneği olarak inşa edildi — `.classify()` onu adlandırıyor, "
                    "`.as_custom_bracket_on_flat_symbols()` Jacopy'nin doğrulanmış `prove_jacobi` "
                    "motorunu besliyor (Sekme I'de kullanılan aynısı)."
                ),
                r"[P,Q]=Z,\qquad [Z,P]=[Z,Q]=0"
            )
            if st.button(L("▶ Build BDCPBracket and verify Jacobi",
                           "▶ BDCPBracket inşa et ve Jacobi'yi doğrula"), key="bdcp_heis"):
                reg_h = BdcpRegistry()
                P = BdcpSymbol("P"); Q = BdcpSymbol("Q"); Z = BdcpSymbol("Z")
                for s in (P, Q, Z):
                    reg_h.declare(s, BdcpGraded(degree=0))

                heisenberg_table = {
                    ("P", "Q"): Z,             ("Q", "P"): BdcpNeg(Z),
                    ("P", "Z"): BdcpInteger(0), ("Z", "P"): BdcpInteger(0),
                    ("Q", "Z"): BdcpInteger(0), ("Z", "Q"): BdcpInteger(0),
                    ("P", "P"): BdcpInteger(0), ("Q", "Q"): BdcpInteger(0), ("Z", "Z"): BdcpInteger(0),
                }
                def psi_heis(x, y, registry):
                    return heisenberg_table.get((str(x), str(y)), BdcpInteger(0))

                heisenberg_bdcp = BDCPBracket(psi=psi_heis, name="[·,·]_g⋈ψh")
                st.info(L(f"Classified as: **{heisenberg_bdcp.classify()}**",
                          f"Sınıflandırma: **{heisenberg_bdcp.classify()}**"))

                bracket_heis = heisenberg_bdcp.as_custom_bracket_on_flat_symbols(
                    heisenberg_table, name="[·,·]_Heisenberg"
                )
                try:
                    chain_heis = bdcp_prove_jacobi(bracket_heis, P, Q, Z, registry=reg_h)
                    st.success(L(f"✓ Jacobi identity proved in {len(chain_heis)} step(s).",
                                 f"✓ Jacobi kimliği {len(chain_heis)} adımda kanıtlandı."))
                    proof_block(chain_heis, "bdcp_heisenberg")
                except BdcpProofFailure as e:
                    fail(e)

        # ── VII.3 aff(1) — built via BDCPBracket, verified via prove_jacobi ──
        with proof_tabs7[2]:
            section_intro(
                L("Double cross / semi-direct product: aff(1) = ℝ ⋊ ℝ",
                  "Double cross / semi-direkt çarpım: aff(1) = ℝ ⋊ ℝ"),
                L(
                    "$\\mathfrak{g}=\\mathrm{span}(H)$, $\\mathfrak{h}=\\mathrm{span}(X)$, both abelian, "
                    "no twisted cocycles, genuine one-sided action $\\rho(X,H)=X$. "
                    "Built as `BDCPBracket(rho=...)` — the affine Lie algebra of the line.",
                    "$\\mathfrak{g}=\\mathrm{span}(H)$, $\\mathfrak{h}=\\mathrm{span}(X)$, ikisi de abelyen, "
                    "bükülmüş kosiklik yok, gerçek tek taraflı etki $\\rho(X,H)=X$. "
                    "`BDCPBracket(rho=...)` olarak inşa edildi — doğrunun afin Lie cebiri."
                ),
                r"[H,X]=X"
            )
            if st.button(L("▶ Build BDCPBracket and verify Jacobi",
                           "▶ BDCPBracket inşa et ve Jacobi'yi doğrula"), key="bdcp_aff"):
                reg_a = BdcpRegistry()
                H = BdcpSymbol("H"); X = BdcpSymbol("X")
                for s in (H, X):
                    reg_a.declare(s, BdcpGraded(degree=0))

                aff1_table = {
                    ("H", "X"): X, ("X", "H"): BdcpNeg(X),
                    ("H", "H"): BdcpInteger(0), ("X", "X"): BdcpInteger(0),
                }
                def rho_aff1(x, y, registry):
                    return aff1_table.get((str(y), str(x)), BdcpInteger(0))

                aff1_bdcp = BDCPBracket(rho=rho_aff1, name="[·,·]_A⋊B")
                st.info(L(f"Classified as: **{aff1_bdcp.classify()}**",
                          f"Sınıflandırma: **{aff1_bdcp.classify()}**"))

                bracket_aff1 = aff1_bdcp.as_custom_bracket_on_flat_symbols(
                    aff1_table, name="[·,·]_aff(1)"
                )
                try:
                    chain_aff1 = bdcp_prove_jacobi(bracket_aff1, H, X, H, registry=reg_a)
                    st.success(L(f"✓ Jacobi identity proved in {len(chain_aff1)} step(s).",
                                 f"✓ Jacobi kimliği {len(chain_aff1)} adımda kanıtlandı."))
                    proof_block(chain_aff1, "bdcp_aff1")
                except BdcpProofFailure as e:
                    fail(e)
        ...
        with proof_tabs7[3]:
            section_intro(
                L("Roadmap to a full BDCP proof engine", "Tam bir BDCP kanıt motoruna yol haritası"),
                L(
                    "`BDCPSection` and `BDCPBracket` (in 32_bdcp_algebroids.py) handle the scalar / "
                    "finite-basis case using only already-verified Jacopy primitives — no new core "
                    "code. Two additions would be needed for full generality:\n\n"
                    "1. **Vector-bundle-valued structure maps** — extending BDCPSection beyond a "
                    "fixed finite basis to genuine sections of vector bundles A, B over a manifold.\n"
                    "2. **A contactization rule** (z-extension) for the dissipative Hamilton's "
                    "equations of §3, analogous to the K/K̃ Cartan-remainder operators in Tab V.\n\n"
                    "Until then, the Hierarchy Explorer and the two worked Lie-algebra instances "
                    "are the honest, currently-provable slice of this paper inside Jacopy.",
                    "`BDCPSection` ve `BDCPBracket` (32_bdcp_algebroids.py'de) yalnızca zaten "
                    "doğrulanmış Jacopy primitifleriyle skaler durumu ele alır — yeni çekirdek kod yok. "
                    "Tam genellik için iki eklenti gerekir:\n\n"
                    "1. **Vektör-demeti değerli yapı dönüşümleri** — BDCPSection'ı sabit sonlu "
                    "bazın ötesine, bir manifold üzerindeki A, B vektör demetlerinin gerçek "
                    "kesitlerine genişletmek.\n"
                    "2. §3'ün yitirgen Hamilton denklemleri için **bir kontaktlaştırma kuralı** "
                    "(z-genişlemesi), Sekme V'teki K/K̃ Cartan-artık operatörlerine benzer.\n\n"
                    "Bunlar eklenene kadar, Hiyerarşi Kaşifi ve iki işlenmiş Lie cebiri örneği, "
                    "bu makalenin Jacopy içinde şu anda dürüstçe kanıtlanabilir dilimidir."
                )
            )

# ══════════════════════════════════════════════════════════════
# TAB VIII — Lie Algebroid Connections & 3-Lie Algebroids
#            Ateşli, Esen & Sütlü, arXiv:2606.25141
#            "Constructions of 3-Lie algebroids"
#
# Uses the real AbstractConnection / TernaryBracketFromXi classes
# defined in 33_lie_algebroid_connections.py — no ad-hoc math
# duplicated here.
# ══════════════════════════════════════════════════════════════
with tabs[7]:
    st.header(L(
        "VIII · Lie Algebroid Connections & 3-Lie Algebroids",
        "VIII · Lie Algebroid Bağlantıları & 3-Lie Algebroidleri"
    ))
    st.caption("Ateşli, Esen & Sütlü — *Constructions of 3-Lie algebroids*, arXiv:2606.25141 (2026)")

    st.markdown(L(
        """This paper's central observation: a Lie algebroid bracket can **always** be produced
        from a **connection** $\\nabla$ on any anchored bundle $\\mathcal{A}$ — not just $TM$ —
        via $[X,Y]^\\nabla := \\nabla_X(Y) - \\nabla_Y(X)$, which automatically satisfies the
        Leibniz identity. A **curvature operator** is then defined, and the Jacobi identity for
        $[\\cdot,\\cdot]^\\nabla$ is shown to be **equivalent** to a Bianchi identity for the
        curvature — exactly generalizing the Bianchi/Jacobi dictionary already used for
        $TM$-connections in Tab IV, but now for an *arbitrary* anchored bundle.

        The paper then lifts this dictionary from binary (Lie algebroid) brackets to **ternary
        (3-Lie / Filippov) brackets**, using generating families of several differential
        operators and dual sections, and applies it to construct a concrete 3-Lie algebroid
        from Poisson Lie algebroid data.""",
        """Bu makalenin merkezi gözlemi: bir Lie algebroid bracket'ı **her zaman** herhangi bir
        anchored bundle $\\mathcal{A}$ üzerindeki (yalnızca $TM$ değil) bir **bağlantıdan**
        $\\nabla$ türetilebilir: $[X,Y]^\\nabla := \\nabla_X(Y) - \\nabla_Y(X)$, bu otomatik
        olarak Leibniz kimliğini sağlar. Sonra bir **eğrilik operatörü** tanımlanır ve
        $[\\cdot,\\cdot]^\\nabla$ için Jacobi kimliğinin, eğrilik için bir Bianchi kimliğine
        **denk** olduğu gösterilir — Sekme IV'te $TM$-bağlantıları için zaten kullanılan
        Bianchi/Jacobi sözlüğünün, *keyfi* bir anchored bundle için genellemesi.

        Makale sonra bu sözlüğü ikili (Lie algebroid) bracket'lardan **üçlü (3-Lie / Filippov)
        bracket'lara** taşır; birden fazla diferansiyel operatör ve dual kesit ailesi kullanarak,
        ve Poisson Lie algebroid verisinden somut bir 3-Lie algebroid inşa eder."""
    ))
    st.latex(r"[X,Y]^\nabla := \nabla_X(Y) - \nabla_Y(X), \qquad \mathcal{R}^\nabla(X,Y)(Z) := \nabla_X\nabla_Y(Z) - \nabla_Y\nabla_X(Z) - \nabla_{[X,Y]^\nabla}(Z)")
    st.latex(r"\text{Jacobi for } [\cdot,\cdot]^\nabla \iff \mathcal{R}^\nabla(X,Y)Z + \mathcal{R}^\nabla(Y,Z)X + \mathcal{R}^\nabla(Z,X)Y = 0")
    st.markdown("---")

    # Import the real classes built in 33_lie_algebroid_connections.py
    import importlib.util as _ilu2
    _lac_path = Path.cwd() / "docs" / "tutorials" / "33_lie_algebroid_connections.py"
    if not _lac_path.is_file():
        _lac_path = here / "33_lie_algebroid_connections.py"
    _spec2 = _ilu2.spec_from_file_location("lac_module", str(_lac_path))
    _lac_module = _ilu2.module_from_spec(_spec2)
    import sys as _sys2
    _sys2.modules["lac_module"] = _lac_module
    try:
        _spec2.loader.exec_module(_lac_module)
        AbstractConnection = _lac_module.AbstractConnection
        TernaryBracketFromXi = _lac_module.TernaryBracketFromXi
        _lac_import_ok = True
    except Exception as _e2:
        _lac_import_ok = False
        _lac_import_error = _e2

    proof_tabs8 = st.tabs([
        L("Connection → Bracket → Bianchi", "Bağlantı → Bracket → Bianchi"),
        L("Recovering the Lie Bracket", "Lie Bracket'ı Geri Kazanmak"),
        L("Ternary (Filippov) Bracket", "Üçlü (Filippov) Bracket"),
        L("What's still needed", "Hâlâ neye ihtiyaç var"),
    ])

    if not _lac_import_ok:
        st.error(L(
            f"Could not load 33_lie_algebroid_connections.py: {_lac_import_error}. "
            "Make sure docs/tutorials/33_lie_algebroid_connections.py exists.",
            f"33_lie_algebroid_connections.py yüklenemedi: {_lac_import_error}. "
            "docs/tutorials/33_lie_algebroid_connections.py dosyasının var olduğundan emin olun."
        ))
    else:
        # ── VIII.1 Connection → Bracket → Bianchi, on an abstract A ──
        with proof_tabs8[0]:
            section_intro(
                L("Trivial connection on an abstract anchored bundle",
                  "Soyut bir anchored bundle üzerinde bedel bağlantı"),
                L(
                    "Build a real `AbstractConnection` with $\\nabla_X(Y):=0$ on an "
                    "arbitrary anchored bundle (not $TM$). Its derived bracket vanishes "
                    "identically, so the Bianchi defect vanishes and Jacobi holds trivially — "
                    "verified via Jacopy's own `prove_jacobi`, wired through "
                    "`AbstractConnection.as_custom_bracket()`.",
                    "Keyfi bir anchored bundle üzerinde (yalnızca $TM$ değil) $\\nabla_X(Y):=0$ "
                    "olan gerçek bir `AbstractConnection` inşa et. Türetilmiş bracket özdeş "
                    "olarak sıfırlanır, Bianchi defekti sıfırlanır ve Jacobi kimliği önemsiz "
                    "biçimde sağlanır — Jacopy'nin `prove_jacobi`'si ile doğrulanır."
                )
            )
            if st.button(L("▶ Build AbstractConnection and verify Jacobi",
                           "▶ AbstractConnection inşa et ve Jacobi'yi doğrula"), key="lac_trivial"):
                from jacopy.core.expr import Symbol as LacSymbol, Integer as LacInteger
                from jacopy.core.properties import Graded as LacGraded
                from jacopy.core.registry import PropertyRegistry as LacRegistry

                reg_t = LacRegistry()
                X_t = LacSymbol("X"); Y_t = LacSymbol("Y"); Z_t = LacSymbol("Z")
                for s in (X_t, Y_t, Z_t):
                    reg_t.declare(s, LacGraded(degree=0))

                def nabla_zero(a, b, registry):
                    return LacInteger(0)

                conn = AbstractConnection(nabla_zero, name="triv")
                br = conn.bracket(X_t, Y_t, reg_t)
                defect = conn.bianchi_defect(X_t, Y_t, Z_t, reg_t)
                st.write(f"**[X,Y]^∇ =** `{to_ascii(br)}`")
                st.write(f"**Bianchi defect =** `{to_ascii(defect)}`")

                bracket_t = conn.as_custom_bracket(satisfies_graded_jacobi=True)
                try:
                    chain_t = bdcp_prove_jacobi(bracket_t, X_t, Y_t, Z_t, registry=reg_t)
                    st.success(L(f"✓ Jacobi identity proved in {len(chain_t)} step(s).",
                                 f"✓ Jacobi kimliği {len(chain_t)} adımda kanıtlandı."))
                    proof_block(chain_t, "lac_trivial")
                except BdcpProofFailure as e:
                    fail(e)

        # ── VIII.2 Recovering the Lie bracket ─────────────────────
        with proof_tabs8[1]:
            section_intro(
                L("Every Lie algebroid bracket comes from a connection",
                  "Her Lie algebroid bracket'ı bir bağlantıdan gelir"),
                L(
                    "Take $\\nabla_X(Y) := \\tfrac{1}{2}[X,Y]_{\\mathrm{Lie}}$. Then "
                    "$[X,Y]^\\nabla = \\nabla_X(Y) - \\nabla_Y(X) = [X,Y]_{\\mathrm{Lie}}$ "
                    "exactly — recovering the ordinary Lie bracket, illustrating that "
                    "**every** Lie algebroid bracket arises from *some* connection "
                    "(the converse direction of the paper's dictionary).",
                    "$\\nabla_X(Y) := \\tfrac{1}{2}[X,Y]_{\\mathrm{Lie}}$ alalım. O zaman "
                    "$[X,Y]^\\nabla = [X,Y]_{\\mathrm{Lie}}$ tam olarak elde edilir — "
                    "standart Lie bracket'ı geri kazanır, **her** Lie algebroid bracket'ının "
                    "*bir* bağlantıdan geldiğini gösterir."
                ),
                r"\nabla_X(Y) := \tfrac{1}{2}[X,Y]_{\mathrm{Lie}} \;\Longrightarrow\; [X,Y]^\nabla = [X,Y]_{\mathrm{Lie}}"
            )
            if st.button(L("▶ Recover the Lie bracket from a half-Lie connection",
                           "▶ Half-Lie bağlantısından Lie bracket'ı geri kazan"), key="lac_recover"):
                from jacopy.core.expr import Symbol as LacSymbol2
                from jacopy.core.properties import Graded as LacGraded2
                from jacopy.core.registry import PropertyRegistry as LacRegistry2
                from jacopy.brackets.lie import lie as lac_lie

                reg_r = LacRegistry2()
                X_r = LacSymbol2("X"); Y_r = LacSymbol2("Y")
                for s in (X_r, Y_r):
                    reg_r.declare(s, LacGraded2(degree=0))

                def nabla_half(a, b, registry):
                    try:
                        return lac_lie.expand(a, b, registry)
                    except Exception:
                        return Integer(0)

                conn_r = AbstractConnection(nabla_half, name="half-Lie")
                recovered_r = conn_r.bracket(X_r, Y_r, reg_r)
                st.success(L("✓ Bracket recovered.", "✓ Bracket geri kazanıldı."))
                st.write(f"**[X,Y]^∇ (recovered) =** `{to_ascii(recovered_r)}`")

        # ── VIII.3 Ternary (Filippov) bracket ──────────────────────
        with proof_tabs8[2]:
            section_intro(
                L("A ternary (Filippov) bracket from a Lie bracket + trace functional",
                  "Lie bracket + iz fonksiyoneli'nden üçlü (Filippov) bracket"),
                L(
                    "$[X,Y,Z]_\\xi := \\xi(X)[Y,Z] + \\xi(Y)[Z,X] + \\xi(Z)[X,Y]$ turns any "
                    "Lie bracket plus an invariant linear functional $\\xi$ into a genuine "
                    "Filippov 3-bracket. Built here as an actual `TernaryBracketFromXi` "
                    "instance on $\\mathfrak{so}(3)$.",
                    "$[X,Y,Z]_\\xi := \\xi(X)[Y,Z] + \\xi(Y)[Z,X] + \\xi(Z)[X,Y]$, herhangi bir "
                    "Lie bracket'ı ve değişmez bir doğrusal fonksiyonel $\\xi$'yi gerçek bir "
                    "Filippov 3-bracket'ına dönüştürür. $\\mathfrak{so}(3)$ üzerinde gerçek bir "
                    "`TernaryBracketFromXi` örneği olarak inşa edildi."
                ),
                r"[X,Y,Z]_\xi := \xi(X)[Y,Z] + \xi(Y)[Z,X] + \xi(Z)[X,Y]"
            )
            if st.button(L("▶ Build TernaryBracketFromXi on so(3)",
                           "▶ so(3) üzerinde TernaryBracketFromXi inşa et"), key="lac_ternary"):
                from jacopy.core.expr import Symbol as LacSymbol3, Neg as LacNeg3, Integer as LacInteger3
                from jacopy.core.properties import Graded as LacGraded3
                from jacopy.core.registry import PropertyRegistry as LacRegistry3

                reg_s = LacRegistry3()
                e1_s = LacSymbol3("e1"); e2_s = LacSymbol3("e2"); e3_s = LacSymbol3("e3")
                for s in (e1_s, e2_s, e3_s):
                    reg_s.declare(s, LacGraded3(degree=0))

                so3_table_app = {
                    ("e1", "e2"): e3_s, ("e2", "e1"): LacNeg3(e3_s),
                    ("e2", "e3"): e1_s, ("e3", "e2"): LacNeg3(e1_s),
                    ("e3", "e1"): e2_s, ("e1", "e3"): LacNeg3(e2_s),
                    ("e1", "e1"): LacInteger3(0), ("e2", "e2"): LacInteger3(0), ("e3", "e3"): LacInteger3(0),
                }
                def so3_br_app(a, b, registry):
                    return so3_table_app.get((str(a), str(b)), LacInteger3(0))
                def xi_const_app(a, registry):
                    return LacInteger3(1)

                ternary = TernaryBracketFromXi(so3_br_app, xi_const_app, name="[·,·,·]_so3")
                val_app = ternary.expand(e1_s, e2_s, e3_s, reg_s)
                val_swap_app = ternary.expand(e2_s, e1_s, e3_s, reg_s)
                st.success(L("✓ Ternary bracket computed.", "✓ Üçlü bracket hesaplandı."))
                c1, c2 = st.columns(2)
                c1.write("**[e1,e2,e3]_ξ:**"); c1.code(to_ascii(val_app))
                c2.write("**[e2,e1,e3]_ξ:**"); c2.code(to_ascii(val_swap_app))
                st.caption(L("Antisymmetry under the first swap is visible by direct comparison.",
                             "İlk değişimde antisimetri doğrudan karşılaştırmayla görülebilir."))

        # ── VIII.4 Roadmap ──────────────────────────────────────────
        with proof_tabs8[3]:
            section_intro(
                L("Roadmap to a full 3-Lie algebroid proof engine",
                  "Tam bir 3-Lie algebroid kanıt motoruna yol haritası"),
                L(
                    "`AbstractConnection` (Part 1) faithfully reproduces the paper's core "
                    "connection ↔ bracket ↔ curvature ↔ Bianchi/Jacobi dictionary for an "
                    "arbitrary anchored bundle, using Jacopy's already-verified "
                    "`CustomBracket` + `prove_jacobi`. `TernaryBracketFromXi` (Part 2) gives "
                    "a genuine, self-contained Filippov 3-bracket, but is **not yet** the "
                    "paper's general multi-operator generating-family construction needed "
                    "for the Jacobi Lie algebroid and the Poisson 3-Lie algebroid application. "
                    "Two additions would be needed:\n\n"
                    "1. A **native ternary CustomBracket + prove_fundamental_identity engine** "
                    "— Jacopy currently only has a binary version.\n"
                    "2. The paper's precise **multi-operator generating-family compatibility "
                    "conditions**, letting Jacopy reconstruct the Jacobi Lie algebroid bracket "
                    "and the Poisson 3-Lie algebroid exactly.\n\n"
                    "Until then, the connection dictionary and the worked Filippov example are "
                    "the honest, currently-provable slice of this paper inside Jacopy.",
                    "`AbstractConnection` (Bölüm 1), makalenin temel bağlantı ↔ bracket ↔ "
                    "eğrilik ↔ Bianchi/Jacobi sözlüğünü keyfi bir anchored bundle için sadakatle "
                    "yeniden üretir. `TernaryBracketFromXi` (Bölüm 2) gerçek bir Filippov "
                    "3-bracket'ı verir, ama **henüz** makalenin genel çok-operatörlü inşasını "
                    "değildir. İki eklenti gerekir:\n\n"
                    "1. **Yerel üçlü CustomBracket + fundamental identity motoru**.\n"
                    "2. Makalenin **çok-operatörlü uyumluluk koşulları**.\n\n"
                    "Bunlar eklenene kadar, bağlantı sözlüğü ve işlenmiş Filippov örneği, bu "
                    "makalenin Jacopy içinde şu anda dürüstçe kanıtlanabilir dilimidir."
                )
            )